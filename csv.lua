...
function file_buffer:truncate(p)
  ...
  if p > self.buffer_block_size then
    local remove = self.buffer_block_size * math.floor((p - 1) / self.buffer_block_size)
...
function file_buffer:find(pattern, init)
  while true do
    local first, last, capture = self.buffer:find(pattern, init - self.buffer_start)
...
function file_buffer:extend(offset)
  ...
  if extra > 0 then
    local size = self.buffer_block_size * math.ceil(extra / self.buffer_block_size)
...
local function try_separator(buffer, sep, f)
  ...
  local iterator =
    coroutine.wrap(
    function()
      f(buffer, guess_separator_params)
    end
  )
...
local function separated_values_iterator(buffer, parameters)
  ...
  local sep = "([" .. (parameters.separator or guess_separator(buffer, separated_values_iterator)) .. "\n\r])"
  local line_start = 1
  local line = 1
  ...
  local function problem(message)
    error(("%s:%d:%d: %s"):format(parameters.filename, field_start_line, field_start_column, message), 0)
  end
  ...
  while true do
    ...
    if parameters.column_map and header_read then
      local ok
      ok, value, key = pcall(parameters.column_map.transform, parameters.column_map, value, field_count)
      if not ok then
        problem(value)
      end
    elseif header then
