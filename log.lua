...
local fileLog = string.format("/Log/%s/%04d-%02d-%02d.log", Broker, datetime.year, datetime.month, datetime.day)
log.outfile = fileLog
...
for i, x in ipairs(modes) do
  ...
  log[x.name] = function(...)
    ...
    -- Output to log file
    if log.outfile then
      local fp = io.open(getScriptPath() .. log.outfile, "a")
      local str =
        string.format("%-6s\t\t%s\t\t%s\t\t%s:\t\t%s\n", nameupper, os.date("%x %X", os.time()), Broker, lineinfo, msg)
      fp:write(str)
      fp:close()
    end
  end
